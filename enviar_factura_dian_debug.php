...
// Script para depurar el proceso de apertura y envío de facturas a la DIAN
// Incluye información detallada de depuración y formato alternativo para abrir facturas
...
if (!$idFactura) {
    ...
    echo "Uso: php enviar_factura_dian_debug.php ID_FACTURA\n";
    exit(1);
}
...
function abrirFactura($idFactura, $credenciales) {
    ...
    // Datos para abrir la factura - formato alternativo más completo
    $datos = json_encode([
        'paymentForm' => 'CASH',
        'paymentMethod' => 'CASH',
        'payment' => [
    ...
    // Esperar un momento para que Alegra procese el cambio
    echo "Esperando 2 segundos para que Alegra procese el cambio...\n";
    sleep(2);
...
// Proceso principal
echo "=== Proceso de envío de factura a la DIAN (Modo Debug) ===\n";
...
if ($estadoActual === 'draft') {
    ...
    if (!abrirFactura($idFactura, $credenciales)) {
        echo "No se pudo abrir la factura. Proceso abortado.\n";
        exit(1);
    }
    ...
    if ($nuevoEstado !== 'open') {
        echo "La factura no cambió a estado abierto después de intentar abrirla. Proceso abortado.\n";
        exit(1);
    }
