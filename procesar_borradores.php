...
// IDs de facturas a verificar (los últimos IDs que probablemente contengan borradores)
// Estos IDs pueden ser ajustados según sea necesario
$idsFacturas = range(120, 100, -1); // Verificar facturas desde ID 120 hasta 100

echo "\nBuscando facturas en estado borrador...\n";
echo str_repeat('-', 100) . "\n";
...
foreach ($idsFacturas as $idFactura) {
    ...
    // Si hay error al consultar, continuar con la siguiente
...
// Verificar si se encontraron facturas en estado borrador
if (count($facturasBorrador) === 0) {
    echo "No se encontraron facturas en estado borrador.\n";
    exit(0);
}

echo "\nSe encontraron " . count($facturasBorrador) . " facturas en estado borrador.\n";
