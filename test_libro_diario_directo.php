<?php

require_once __DIR__ . '/vendor/autoload.php';

// Inicializar la aplicación Laravel
$app = require_once __DIR__ . '/bootstrap/app.php';
$kernel = $app->make(Illuminate\Contracts\Console\Kernel::class);
$kernel->bootstrap();

use App\Http\Controllers\Contabilidad\ReporteContableController;
use Illuminate\Http\Request;
use Carbon\Carbon;

echo "=== PRUEBA DIRECTA LIBRO DIARIO ===\n\n";

try {
    $controller = $app->make(ReporteContableController::class);
    
    echo "📚 Probando método libro_diario...\n";
    
    $request = new Request([
        'fecha_desde' => Carbon::now()->startOfMonth()->format('Y-m-d'),
        'fecha_hasta' => Carbon::now()->format('Y-m-d')
    ]);
    
    $response = $controller->libro_diario($request);
    
    if ($response instanceof \Illuminate\View\View) {
        echo "  ✅ Vista generada correctamente\n";
        echo "  📄 Vista: " . $response->name() . "\n";
        
        $data = $response->getData();
        echo "  📊 Datos disponibles:\n";
        foreach (array_keys($data) as $key) {
            echo "    - {$key}\n";
        }
        
        if (isset($data['comprobantes'])) {
            echo "  📋 Comprobantes encontrados: " . $data['comprobantes']->count() . "\n";
        }
        
        if (isset($data['totales'])) {
            echo "  💰 Totales generales:\n";
            echo "    - Cantidad: " . $data['totales']['general']['cantidad'] . "\n";
            echo "    - Débitos: $" . number_format($data['totales']['general']['debitos'], 2) . "\n";
            echo "    - Créditos: $" . number_format($data['totales']['general']['creditos'], 2) . "\n";
        }
        
    } else {
        echo "  ❌ Respuesta no es una vista\n";
        echo "  📝 Tipo de respuesta: " . get_class($response) . "\n";
    }
    
} catch (\Exception $e) {
    echo "  ❌ Error: " . $e->getMessage() . "\n";
    echo "  📍 Línea: " . $e->getLine() . "\n";
    echo "  📁 Archivo: " . basename($e->getFile()) . "\n";
}

echo "\n🔗 URL directa: http://127.0.0.1:8000/contabilidad/reportes/libro-diario\n";
echo "🔗 Ruta nombrada: reportes.libro-diario\n";

// Verificar que la vista existe
$vistaPath = resource_path('views/contabilidad/reportes/libro_diario.blade.php');
if (file_exists($vistaPath)) {
    echo "✅ Vista existe en: {$vistaPath}\n";
} else {
    echo "❌ Vista NO existe en: {$vistaPath}\n";
}

echo "\n✅ Prueba completada.\n";
